...
| Succeeded helper Job cleanup | n/a | n/a | Partial | API | | Opt-in `spec.global.jobs.succeededHistoryTTLSeconds` cleans completed output-backed enrollment, channel block, and chaincode lifecycle Jobs while retaining failures plus join/anchor proof Jobs |
