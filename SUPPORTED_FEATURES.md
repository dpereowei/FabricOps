...
| Succeeded helper Job cleanup | n/a | n/a | Partial | API | | Opt-in `spec.global.jobs.succeededHistoryTTLSeconds` cleans completed output-backed enrollment, channel block, orderer join, and chaincode lifecycle Jobs while retaining failures plus peer/anchor proof Jobs |
